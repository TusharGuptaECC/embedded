...
bool interrupt_switch_pressed(void)
{
	...
	*/
	int status;
	status = interrupt_sw_pin_reg;
	status = interrupt_sw_pin_reg & ( 1 << interrupt_sw_pin );
	if (status == 1 << interrupt_sw_pin)
	{
		return false;
	} 
	else
 ...
 */
void turn_on_bar_graph_led(unsigned int led_pin) {
	// << NOTE >> : Use Masking and Shift Operators here
 ...
 * @param[in]  led_pin   Pin number of Bar-graph LED
 */
void turn_off_bar_graph_led(unsigned int led_pin) {
	// << NOTE >> : Use Masking and Shift Operators here
