...
void motors_pin_config(void) {
	...
	motors_dir_ddr_reg |= ( ( 1 << motors_RF_pin ) | ( 1 << motors_RB_pin ) | ( 1 << motors_LF_pin ) | ( 1 << motors_LB_pin ) );			// set **ONLY** motor pins as output
	motors_dir_port_reg &= ~( ( 1 << motors_RF_pin ) | ( 1 << motors_RB_pin ) | ( 1 << motors_LF_pin ) | ( 1 << motors_LB_pin ) );			// stop motors initially
}
...
void timer_pwm_init(void) {
	...
	TCCR5A_reg = 0xA1;	// 1010,0001

	// Configure for Phase Correct PWM
	// Set Prescalar to 64
	TCCR5B_reg = 0x03;	// 0000,0011
